...
export default function Sidebar({ open, onClose, user }: SidebarProps) {
  ...
  useEffect(() => {
    ...
    const loadCustomNavigation = async () => {
      try {
        const publicRequest = fetch("/api/navigation", { cache: "no-store" });
        const adminRequest = user?.role === "admin"
          ? fetch("/api/admin/navigation", { cache: "no-store" })
          : Promise.resolve(null);
        const [publicResponse, adminResponse] = await Promise.all([publicRequest, adminRequest]);
        if (!publicResponse.ok) throw new Error("PUBLIC_NAVIGATION_FAILED");
