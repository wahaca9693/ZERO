...
export default function AsiacellDepositPage() {
  ...
  const startTransfer = async (event: React.FormEvent) => {
    ...
    if (!Number.isSafeInteger(amount) || amount < 1000 || amount % 1000 !== 0) {
      setMessage("مبلغ التحويل يجب أن يكون 1000 د.ع أو مضاعفاته");
      return;
    }
    setLoading(true);
    setMessage("");
