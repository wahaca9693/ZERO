...
export default function AutoRefillPage({ slug }: { slug: string }) {
  ...
  const fetchData = useCallback(async () => {
    try {
      const [sRes, rRes] = await Promise.all([fetch(`/api/sites/${slug}/services`), fetch(`/api/sites/${slug}/auto-refill`)]);
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    const res = await fetch(`/api/sites/${slug}/auto-refill`, {
      method: "POST",
  ...
  const remove = async (id: number) => {
    ...
    await fetch(`/api/sites/${slug}/auto-refill?id=${id}`, { method: "DELETE" });
    fetchData();
  };
