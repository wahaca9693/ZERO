...
export default function FreeServicesPage({ slug }: { slug: string }) {
  ...
  const loadOffers = async () => {
    setLoading(true);
    try {
      const response = await fetch(`/api/sites/${slug}/free-services`, { cache: "no-store" });
  ...
  const redeem = async (offer: FreeOffer) => {
    ...
    try {
      ...
      const response = await fetch(`/api/sites/${slug}/free-services/redeem`, { method: "POST", headers: { "Content-Type": "application/json", "Idempotency-Key": idempotencyKey }, body: JSON.stringify({ offerId: offer.id, link, quantity, idempotencyKey }) });
