...
export default function AsiacellAdminPage({ slug }: { slug: string }) {
  ...
  const fetchStatus = useCallback(async () => {
    try {
      const res = await fetch(`/api/sites/${encodeURIComponent(slug)}/admin/asiacell-gateway`, { cache: "no-store" });
  ...
  useEffect(() => {
    fetch(`/api/sites/${encodeURIComponent(slug)}/auth/me`)
      .then((res) => res.json())
  ...
  const login = async (e: React.FormEvent) => {
    ...
    setMessage("");
    try {
      const res = await fetch(`/api/sites/${encodeURIComponent(slug)}/admin/asiacell-gateway`, {
        method: "POST",
