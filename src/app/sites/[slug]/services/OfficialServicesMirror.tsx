...
export default function OfficialServicesMirror({ slug }: { slug?: string }) {
  const router = useRouter();
  const searchParams = useSearchParams();
  // API base: main platform uses /api, branch uses /api/sites/{slug}
  const apiBase = slug ? `/api/sites/${encodeURIComponent(slug)}` : "/api";
  // Page base for links: main "/", branch "/sites/{slug}"
  const pageBase = slug ? "/sites/" + encodeURIComponent(slug) : "";
  ...
  const refreshAccountBalance = useCallback(async () => {
    ...
    try {
      const response = await fetch(`${apiBase}/user`, { cache: "no-store", credentials: "include" });
  ...
  const loadFavorites = useCallback(async () => {
    ...
    try {
      const response = await fetch(`${apiBase}/user/favorites`, { credentials: "include", cache: "no-store" });
      if (!response.ok) return;
  ...
  const toggleFavorite = useCallback(async (serviceId: string) => {
    if (accountAuthState !== "authenticated") {
      ...
      router.push(`${pageBase}/login?next=${encodeURIComponent(next)}`);
      return;
    }
  ...
  const copyServiceLink = useCallback(async (serviceId: string) => {
    const url = new URL(`${pageBase}/services?service=${encodeURIComponent(serviceId)}`, window.location.origin).toString();
    try {
  ...
  useEffect(() => {
    let active = true;
    void fetch(`${apiBase}/catalog-platforms`, { cache: "no-store" })
  ...
  return (
    <div className="relative flex min-h-screen flex-col bg-[var(--color-bg)]">
      {accountAuthState !== "checking" && (
        <>
          <Header onMenuClick={() => setSidebarOpen(true)} user={accountUser} unreadNotifications={0} basePath={pageBase} />
          <Sidebar open={sidebarOpen} onClose={() => setSidebarOpen(false)} user={accountUser} basePath={pageBase} />
        </>
      )}
