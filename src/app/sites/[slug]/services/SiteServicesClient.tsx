...
import { Search, Package, Clock, ShoppingCart, Loader2, X, CheckCircle2, AlertCircle, LogIn, UserPlus, ArrowRight } from "lucide-react";
...
export default function SiteServicesClient({ slug, siteName, logoUrl, primary, secondary }: Props) {
    ...
    const load = async () => {
      try {
        const [res, meRes] = await Promise.all([
          fetch(`/api/sites/${encodeURIComponent(slug)}/services`, { cache: "no-store" }),
  ...
  const openOrder = (service: Service) => {
    if (loggedIn === false) {
      router.push(`/sites/${encodeURIComponent(slug)}/login?next=${encodeURIComponent(`/sites/${encodeURIComponent(slug)}/services`)}`);
      return;
    }
    setSelected(service);
    setLink("");
  ...
  return (
    <div
      ...
      style={{ "--color-primary": primary, "--color-gold": secondary } as React.CSSProperties}
    >
      {/* Top bar — mirror of official platform */}
      <header className="sticky top-0 z-40 border-b border-white/5 bg-[#0b0b09]/85 backdrop-blur-xl">
        <div className="mx-auto flex max-w-6xl items-center justify-between px-4 py-3">
          ...
          <div className="flex items-center gap-2">
            {loggedIn ? (
              <Link
                href={`/sites/${encodeURIComponent(slug)}/dashboard`}
                className="flex items-center gap-1.5 rounded-xl bg-[var(--color-primary)] px-4 py-2 text-sm font-black text-black hover:brightness-110"
              >
                لوحتي <ArrowRight size={15} />
              </Link>
            ) : (
              <>
                <Link
                  href={`/sites/${encodeURIComponent(slug)}/login`}
                  className="rounded-xl border border-white/10 bg-white/5 px-4 py-2 text-sm font-bold text-zinc-200 hover:bg-white/10"
                >
                  تسجيل الدخول
                </Link>
                <Link
                  href={`/sites/${encodeURIComponent(slug)}/login?mode=register`}
                  className="flex items-center gap-1.5 rounded-xl bg-[var(--color-primary)] px-4 py-2 text-sm font-black text-black hover:brightness-110"
                >
      ...
      <main className="mx-auto max-w-6xl px-4 py-6 pb-16">
        {/* Hero — compact, like official services page header */}
        <div className="mb-6 rounded-3xl border border-[var(--color-gold)]/25 bg-gradient-to-br from-[#2e210b] to-[#1e1506] p-6">
          <h1 className="text-2xl font-black text-white">متجر الخدمات — {siteName}</h1>
          <p className="mt-1 text-sm text-zinc-400">تصفح واطلب أفضل الخدمات الرقمية بنفس أسعار المنصة الرسمية</p>
        </div>
        ...
        {categories.length > 0 && (
          <div className="mb-5 flex gap-2 overflow-x-auto pb-1">
            <button
              ...
              className={`shrink-0 rounded-full px-3.5 py-1.5 text-xs font-bold transition ${activeCategory === "all" ? "bg-[var(--color-primary)] text-black" : "bg-white/5 text-zinc-400"}`}
            >
              الكل
            </button>
            {categories.map((category) => (
              <button
                ...
                className={`shrink-0 rounded-full px-3.5 py-1.5 text-xs font-bold transition ${activeCategory === category ? "bg-[var(--color-primary)] text-black" : "bg-white/5 text-zinc-400"}`}
              >
                {category}
              </button>
            ))}
        ...
        {/* Services grid — same card style as official platform */}
