...
import { Search, Filter, MessageSquare, Clock, CheckCircle2, AlertCircle, Loader2, MoreVertical, MessageSquare as MessageSquareIcon } from "lucide-react";
...
export default function TicketsPage({ slug, siteName, primary, secondary, expired }: Props) {
            ...
            <tbody>
              {tickets.map((ticket) => (
                <tr key={ticket.id} className="border-b border-white/10 hover:bg-white/5">
                  ...
                  <td className="px-6 py-4 text-right">
                    <Link href={`/sites/${slug}/tickets/${ticket.id}`} className="p-2 rounded-lg bg-white/5 text-zinc-400 hover:bg-white/10 hover:text-white transition-colors">
