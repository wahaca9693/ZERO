...
export async function POST(request: Request) {
  try {
    ...
    try {
      ...
      console.warn("Could not read registration settings, defaulting to enabled", {
        errorName: error instanceof Error ? error.name : "UnknownError",
      });
    }
    ...
    try {
      result = await db.execute({
        sql: "INSERT INTO users (username, email, password_hash, balance, role, terms_accepted, email_verified, auth_provider, is_2fa_enabled, two_fa_user_configured, security_code_hash) VALUES (?, ?, ?, 0, 'user', 1, 1, 'password', 0, 0, NULL)",
    ...
    try {
      await registrationDbExecute(
        { sql: "INSERT INTO notifications (user_id, title, body) VALUES (?, ?, ?)", args: [userId, "مرحباً بك!", "تم إنشاء حسابك بنجاح. اقرأ شروط الاستخدام قبل الطلب."] },
        "create-welcome-notification",
      );
    } catch (error: unknown) {
      console.warn("Welcome notification was skipped", { errorName: error instanceof Error ? error.name : "UnknownError" });
    }
    ...
    return NextResponse.json({
      user: { id: userId, username, role: "user", balance: 0, emailVerified: true },
      requires2fa: false,
    });
  } catch (error: unknown) {
    console.error("Register error", { errorName: error instanceof Error ? error.name : "UnknownError" });
