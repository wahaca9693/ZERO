...
function sitePayload(row: Row, customers: number) { return { id: Number(row.id), slug: String(row.slug), displayName: String(row.display_name), status: String(row.status), subscriptionStatus: String(row.subscription_status), nextBillingAt: row.next_billing_at ?? null, theme: jsonValue(row.theme_json, {}), paymentMethods: jsonValue(row.payment_methods_json, []), providerAccessEnabled: Number(row.provider_access_enabled ?? 0) === 1, customers }; }
...
export async function GET(request: Request) {
  try {
    ...
    return NextResponse.json({ site: sitePayload(row, Number((count.rows[0] as Row | undefined)?.count || 0)) });
  } catch (error: unknown) {
