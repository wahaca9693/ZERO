...
type SiteItem = { id: number; slug: string; displayName: string; status: string; subscriptionStatus: string; nextBillingAt: string | null; createdAt: string | null; providerAccessEnabled: boolean };
...
export default function ResellerPage() {
  ...
  const createSite = async (event: React.FormEvent) => {
    ...
    try {
      const response = await fetch("/api/reseller", { method: "POST", headers: { "Content-Type": "application/json", "Idempotency-Key": makeCreationKey() }, body: JSON.stringify({ action: "create", slug: normalized, display_name: displayName.trim(), creation_key: makeCreationKey() }) });
